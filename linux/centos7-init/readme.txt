...
然后执行install.start文件
3.install_jdk.sh 是Linux系统（基于centeros 7）安装 jdk (这里使用8u202版本 --作为Oracle JDK的这个版本是最终的免费商用版)的启动脚本
...
4.install_maven.sh 是Linux（基于centeros 7） 安装maven（以3.6.3为例） 的启动脚本
